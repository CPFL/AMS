...
if __name__ == '__main__':
    ...
    dispatcher = Dispatcher(initials["config"], initials["status"], args.state_machine_path)
    dispatcher.set_rate(args.event_loop_rate)
    dispatcher.set_clients(kvs_client, pubsub_client, maps_client)
    try:
        dispatcher.start()
    except KeyboardInterrupt:
        logger.info("dispatcher node: KeyboardInterrupt")
